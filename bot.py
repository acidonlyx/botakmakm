...
# Новая шкала кэшбэка от оборота
def get_cashback_rate(turnover):
    if turnover >= 1000000:
        return 7, 1000000, "Максимальный уровень кэшбэка достигнут (7%)!"
...
# Пример расчет скидки (можно настроить под ваши пороги)
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "Заполните данные вашего авто в Mini App, чтобы они автоматически подтягивались при записи и оценке ремонта.",
...
async def ref_menu_callback(callback: Message):
    ...
    text = (
        ...
        f"👥 Приглашено друзей: **{user.get('active_refs', 0)} чел.**\n\n"
...
async def admin_register_visit(message: Message):
    ...
    args = message.text.split()
    if len(args) < 3:
        return await message.answer("Формат: `/visit ID сумма да/нет`", parse_mode="Markdown")
    ...
    if is_first_visit and not user.get("first_visit_done", False):
        ...
        if referrer_id and referrer_id in db:
            ...
            try:
                await bot.send_message(int(referrer_id), f"🎉 Ваш друг совершил первый визит! Вам начислено *{ref_bonus:,.0f} бонусов*.", parse_mode="Markdown")
            except:
                pass

    user["turnover"] += amount
    save_db(db)
    await message.answer(f"✅ Визит зарегистрирован! Оборот клиента обновлен.")
...
async def handle_api_user_data(request):
    ...
    if not user_id or user_id not in db:
        return web.json_response({
            ...
            "ref_count": 0,
            "bonus_balance": 750,
            "next_text": "Осталось до 2%: 100 000 ₽",
            "car": {"brand": "", "model": "", "vin": "", "plate": ""}
        })
    
    user = db[user_id]
    discount = calculate_discount(user["turnover"])
    cashback_rate, _, next_text = get_cashback_rate(user["turnover"])
